...
export async function GET(
  ...
  try {
    ...
    const relatedRows = db.prepare('SELECT related_id FROM product_related WHERE product_id = ?').all(productId) as { related_id: number }[];
    const related_ids = relatedRows.map(r => r.related_id);

    // Get variants
    const variants = db.prepare('SELECT * FROM product_variants WHERE product_id = ? ORDER BY sort_order, id').all(productId) as Variant[];

    // Get gallery images
    const images = db.prepare('SELECT * FROM product_images WHERE product_id = ? ORDER BY sort_order, id').all(productId) as ProductImage[];

    // Get specs
    const specs = db.prepare('SELECT * FROM product_specs WHERE product_id = ? ORDER BY sort_order, id').all(productId) as ProductSpec[];
