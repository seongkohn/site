...
export async function GET(request: NextRequest) {
  try {
    ...
    if (category) {
      ...
      params.push(parseInt(category, 10), parseInt(category, 10));
    }
    ...
    if (manufacturer) {
      ...
      params.push(parseInt(manufacturer, 10));
    }
...
export async function POST(request: Request) {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO products (name_en, name_ko, slug, sku, category_id, type_id, manufacturer_id,
        description_en, description_ko, features_en, features_ko, image, is_published, is_featured)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      name_en, name_ko, slug, sku,
      category_id || null, type_id || null, manufacturer_id || null,
